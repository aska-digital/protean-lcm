...
- `plugins/lane_gate/` and `plugins/resource_budget/` stay in the repository, unpackaged, pending a
  separate home. Their tests stay with them.
...
- `plugin.yaml` and the shipped skill declare `author: ASKA Digital`, replacing a false Nous
  Research attribution and the internal codenames used by the two unpackaged slices.
...
- The provenance section states that this plugin is not by Nous Research, not by Voltropy PBC, and
  not by Stephen Schoettler, that no upstream author endorses it, and what the vendored
  resource-budget slice is.
